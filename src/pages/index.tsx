  ...
  return (
    <>
      ...
      <div className="w-full overflow-x-hidden">
        ...
        <div className="w-full flex flex-col md:pl-16 md:h-96 xl:h-[580px] md:relative bg-black text-white justify-center items-center md:items-start md:justify-normal pt-4 xl:pt-10 2xl:pt-16 md:gap-y-3" id="location">
        ...
        <div id="contato" className="w-full flex flex-col sm:flex-row relative py-4 lg:p-0 bg-white text-black md:justify-center">
          <Image src={SaladaLeonidas} alt="Prato de saladas Leonidas" className="w-full hidden lg:flex opacity-60 h-[760px]" />
          <h1 className="hidden md:flex text-4xl xl:text-5xl absolute xl:mt-8 drop-shadow-[0_1px_1px_rgba(0,0,0,8)] lg:text-black">ENTRE EM CONTATO</h1>
          <div className="w-full lg:absolute lg:flex md:gap-x-12 xl:gap-x-24">
            <form onSubmit={helpHook.sendEmail}
              className="w-full flex flex-col justify-center items-center gap-y-2 px-4 pb-8 pt-2 md:mt-16 xl:mt-32 xl:pl-72 md:relative">
              <h2 className="text-lg md:hidden">ENTRE EM CONTATO</h2>
              <input onChange={(event) => helpHook.setName(event.target.value)} placeholder="Nome completo" type="text" className="w-full xl:text-xl focus:ring-0" />
              <input onChange={(event) => helpHook.setEmail(event.target.value)} placeholder="Seu melhor e-mail" type="email" className="w-full xl:text-xl focus:ring-0" />
              <input onChange={(event) => helpHook.setPhone(event.target.value)} placeholder="Seu telefone" value={maskPhone(helpHook.phone)} maxLength={13} type="tel" className="w-full xl:text-xl focus:ring-0" />
              <input onChange={(event) => helpHook.setTopic(event.target.value)} placeholder="Qual o assunto?" type="text" className="w-full xl:text-xl focus:ring-0" />
              <textarea onChange={(event) => helpHook.setMessage(event.target.value)} placeholder="Como podemos te ajudar?" id="" cols={30} rows={10} className="w-full xl:text-xl focus:ring-0"></textarea>
              <button type="submit" className="bg-white text-black p-2 md:absolute -bottom-6 left-32 xl:left-72 border xl:text-xl transition-all duration-300 ease-in-out hover:bg-black hover:text-white hover:border hover:border-black">ENVIAR FORMULÁRIO</button>
            </form>

            <div className="w-full flex flex-col items-center justify-center lg:text-black md:flex-row md:mt-16 xl:mt-32 lg:justify-normal lg:items-start">
              <div className="flex lg:flex-col md:justify-center gap-x-4">
                ...
                <ul>
                  ...
                  <Link href={"https://www.instagram.com/leonidassanduba/"} className="hover:font-bold flex gap-x-2 md:items-center md:text-lg md:gap-x-1 xl:text-xl lg:drop-shadow-[0_1px_1px_rgba(0,0,0,8)]">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-4"><title>instagram</title><path d="M7.8,2H16.2C19.4,2 22,4.6 22,7.8V16.2A5.8,5.8 0 0,1 16.2,22H7.8C4.6,22 2,19.4 2,16.2V7.8A5.8,5.8 0 0,1 7.8,2M7.6,4A3.6,3.6 0 0,0 4,7.6V16.4C4,18.39 5.61,20 7.6,20H16.4A3.6,3.6 0 0,0 20,16.4V7.6C20,5.61 18.39,4 16.4,4H7.6M17.25,5.5A1.25,1.25 0 0,1 18.5,6.75A1.25,1.25 0 0,1 17.25,8A1.25,1.25 0 0,1 16,6.75A1.25,1.25 0 0,1 17.25,5.5M12,7A5,5 0 0,1 17,12A5,5 0 0,1 12,17A5,5 0 0,1 7,12A5,5 0 0,1 12,7M12,9A3,3 0 0,0 9,12A3,3 0 0,0 12,15A3,3 0 0,0 15,12A3,3 0 0,0 12,9Z" className="fill-black" /></svg>
                    @leonidassanduba
                  </Link>
                  <Link href={"https://www.instagram.com/leonidassanduba/"} className="hover:font-bold flex gap-x-2 md:items-center md:text-lg md:gap-x-1 xl:text-xl lg:drop-shadow-[0_1px_1px_rgba(0,0,0,8)]">
        ...
        <footer className="w-full flex items-center justify-center py-2 bg-black text-white md:mt-8 lg:m-0 md:py-4 xl:text-lg">
          <p>Desenvolvido por <Link target="blank" href={"https://github.com/guilhermelena01"}>Guilherme Lena</Link></p>
        </footer>
      </div>
    </>
